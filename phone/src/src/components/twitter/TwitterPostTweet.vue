<template>
	<div class="phone_content">
		<div class="tweet_write">
			<div class="ss">
				<img class="icon" :src="twitterAvatarUrl" style="max-width: 48px; max-height: 48px" />
				<textarea
					class="highlight textarea-input"
			...
			<div class="zrifbif">
				<span v-bind:class="{ highlight: selectedOption === 'tweet_send', tweet_send: true }" @click="tweeter">
					{{
						messageSent ? IntlString("APP_TWITTER_BUTTON_ACTION_TWEETER_SENT") : IntlString("APP_TWITTER_BUTTON_ACTION_TWEETER")
					}}
				</span>
				<span v-bind:class="{ highlight: selectedOption === 'tweet_photo', tweet_photo: true }" @click="postphoto">
...
export default {
	...
	computed: {
		...mapGetters(["IntlString", "useMouse", "enableTakePhoto", "twitterAvatarUrl"]),
	},
	watch: {
