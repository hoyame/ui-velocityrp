...
local requestId = 0
function TriggerServerCallback(eventName, args, cb)
	requestId = requestId + 1

	serverCallbacks[#serverCallbacks + 1] = { id = requestId, cb = cb }

	TriggerServerEvent(eventName, requestId, table.unpack(args))
end

RegisterNetEvent("myDJ:serverEventCallback")
AddEventHandler("myDJ:serverEventCallback", function(requestId, ...)
	local eventKey
	for k,v in pairs(serverCallbacks) do
	...
	serverCallbacks[eventKey].cb(table.unpack({...}))
	serverCallbacks[eventKey] = nil
end)


Citizen.CreateThread(function()
    TriggerServerCallback('myDJ:receiveRunningSongs', {}, function(DJPositions)
    ...
    while true do
        ...
        if isAtDJ then
            ...
            if IsControlJustReleased(0, 38) then
                ...
                TriggerServerCallback('myDJ:requestPlaylistsAndSongs', {}, function(playlists, songs)
...
function playTitleFromPlaylist(DJName, DJPos, DJRange, link, playlistId)
    TriggerServerCallback('myDJ:requestPlaylistById', {}, function(playlistSongs)
...
function ShowNotification(text)
	TriggerEvent("gm:player:localnotify", text, "", 0, 10000)
end
