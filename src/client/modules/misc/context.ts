...
export abstract class Context {
	public static Cache: {
		id: number | boolean | number[] | { x: number; y: number };
		type: number | boolean | number[] | { x: number; y: number };
		model: number;
		networkId: number;
		svId: number;
		ifMyplayer: boolean;
	...
	public static async initialize() {
		...
		setTick(() => {
			if (this.Data.focus) {
				...
				if (!this.Data.open) {
					const [isFound, entityCoords, surfaceNormal, entityHit, entityType, cameraDirection, mouse] = this.getScreenToWorld(
						35.0,
						31
					);
	...
	public static getScreenToWorld(d: any, f: any) {
		...
		return [
			...
			surfaceNormal,
			entityHit,
			(entityHit >= 1 && GetEntityType(entityHit)) || 0,
			direction,
			mouse,
		];
	}
